...
public class MostrarAlumno extends ActionBarActivity {
    ...
    public void enviarCorreo(View v){
        final CharSequence correos[]={correo1.getText(),correo2.getText()};

        AlertDialog.Builder dialog = new AlertDialog.Builder(this);
        dialog.setTitle("E-mail? Elije un Correo!");
        dialog.setItems(correos, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                String to[] ={ "Escriba_suCorreo@xx.xx",correos[which].toString()};
                Intent emailIntent = new Intent(Intent.ACTION_SEND);
                emailIntent.setData(Uri.parse("mailto:"));
                emailIntent.putExtra(Intent.EXTRA_EMAIL,to);
                emailIntent.putExtra(Intent.EXTRA_SUBJECT, "EjemploTarea");
                emailIntent.putExtra(Intent.EXTRA_TEXT, "Escriba aqui su mensaje");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        try {
            Class<?> clase = Class.forName("pdm.fia.ues.sv.serviciosocial."+option);
