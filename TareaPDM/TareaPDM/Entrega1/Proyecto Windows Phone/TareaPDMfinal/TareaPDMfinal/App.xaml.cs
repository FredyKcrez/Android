...
namespace TareaPDMfinal
{
    public partial class App : Application
    {
        ...
        public App()
        {
            ...
            string dbPath = Path.Combine(Windows.Storage.ApplicationData.Current.LocalFolder.Path, "db.sqlite1");
            if (!FileExists("db.sqlite1").Result)
            {
                using (var db = new SQLiteConnection(dbPath))
                {
                    db.CreateTable<Proyecto>();
                    db.CreateTable<Encargado>();
                    db.CreateTable<Modalidad>();
                    db.CreateTable<TipoProyecto>();
